/* global wp, wpNavMenu, konteAddonsMegaMenuConfig */
var kmm = kmm || {};

( function( $, _, wp ) {
	hasModified = false;
	var config = window.konteAddonsMegaMenuConfig || {};

	if ( ! config ) {
		return false;
	}

	config.namespace = 'konte_addons_megamenu';
	...
	MegaMenu.prototype.bindEvents = function() {
		...
		self.$modal
			...
			.on( 'change.' + config.namespace, ':input', { megaMenu: self }, self.onInputsChange )
			...
			// Toggle mega menu.
			.on( 'change.' + config.namespace, '[data-field="mega"]', { megaMenu: self }, self.onToggleMegaMenu )
			// Select icon
	...
	MegaMenu.prototype.onClosed = function( event ) {
		...
		self.$modal.find( '.megamenu-modal__content' ).html( '' );

		// Remove 'is-mega' class.
		self.$modal.removeClass( 'is-mega' );

		// Reset wpNavMenu targetList.
		wpNavMenu.targetList = wpNavMenu.menuList;
		$( '.submit-add-to-menu', '#side-sortables' ).prop( 'disabled', false );
		$( '.submit-add-to-menu', '#konte_addons_mega_items').prop( 'disabled', true );
	...
	MegaMenu.prototype.onOpened = function( event ) {
		...
		// Adds an indication class if mega menu option is enabled.
		if( self.itemData.megaData.mega || self.itemData.in_mega ) {
			self.$modal.addClass( 'is-mega' );
		}
	...
	MegaMenu.prototype.onToggleMegaMenu = function( event ) {
		if ( event.currentTarget.checked ) {
			event.data.megaMenu.$modal.addClass( 'is-mega' );
		} else {
			event.data.megaMenu.$modal.removeClass( 'is-mega' );
		}
	}
	...
	MegaMenu.prototype.onSelectIcon = function( event ) {
		...
		$el.closest( '.megamenu-modal__icon-picker' )
			...
			.html( '<i class="' + event.target.dataset.icon + '"></i>' )
			.siblings( 'input[type="hidden"]' ).val( event.target.dataset.icon ).trigger( 'change' );
	}
	...
	MegaMenu.prototype.onSaveChanges = function( event ) {
		...
		$.post( ajaxurl, {
			...
			if ( _.has( data, self.itemData.data['menu-item-db-id'] ) ) {
				var parentItemData = data[ self.itemData.data['menu-item-db-id'] ];

				self.setItemData( self.itemData.element, parentItemData );

				if ( self.itemData.depth === 0 ) {
					if ( parentItemData['mega'] ) {
						self.itemData.element.setAttribute( 'data-mega', parentItemData['mega_mode'] );
					} else {
						self.itemData.element.removeAttribute( 'data-mega' );
					}
				}
			}
	...
	var MegaMenuGridBuilder = function( $grid, data ) {
		...
		self.$ui.on( 'column_added column_removed', { builder: self }, self.toggleRowEmptyClass );
	...
	MegaMenuGridBuilder.prototype.removeRow = function( rowId ) {
		var self = this,
			rowData = _.findWhere( self.data.rows, { id: rowId } );

		self.$content.find( self.selectors.row + '[data-id="' +  rowId + '"]' ).remove();

		self.data.rows = _.without( self.data.rows, rowData );
		...
		self.$ui.trigger( 'row_removed', [ rowData ] );
	}
	...
	MegaMenuGridBuilder.prototype.addColumn = function( colData ) {
		...
		this.$ui.trigger( 'column_added', [ colData ] );
	}

	MegaMenuGridBuilder.prototype.removeColumn = function( colId ) {
		var self = this,
			colData = _.findWhere( self.data.columns, { id: colId } );

		self.$content.find( self.selectors.column + '[data-id="' +  colId + '"]' ).remove();

		self.data.columns = _.without( self.data.columns, colData );
		...
		self.$ui.trigger( 'column_removed', [ colData ] );
	}
	...
	MegaMenuGridBuilder.prototype.removeItem = function( itemId ) {
		var itemData = _.findWhere( this.data.items, { id: itemId } );

		this.data.items = _.without( this.data.items, itemData );
		this.sortItems();
		this.$ui.trigger( 'item_removed', [ itemData ] );
	...
	MegaMenuGridBuilder.prototype.toggleRowEmptyClass = function( event, colData ) {
		var self = event.data.builder,
			$row = self.$content.find( self.selectors.row + '[data-id="' + colData.row + '"]' );

		if ( ! $row.length ) {
			return;
		}

		if ( $row.find( '.megamenu-modal-grid__row-inside' ).children().length ) {
	 ...
	 */
	MegaMenuGridBuilder.prototype.onColumnAdded = function( event, colData ) {
		var self = event.data.builder,
			$column = self.$content.find( self.selectors.column + '[data-id="' + colData.id + '"]' ),
