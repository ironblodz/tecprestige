 ...
 */
$source = $text = $link = $google_fonts = $font_container = $el_id = $el_class = $css = $css_animation = $font_container_data = $google_fonts_data = $separate_link = array();
...
if ( ! empty( $link ) ) {
	...
	if ( $separate_link == 'yes' ) {
		$css_class .= ' separated_link';
		$text      .= '<a href="' . esc_attr( $link['url'] ) . '"' . ( $link['target'] ? ' target="' . esc_attr( $link['target'] ) . '"' : '' ) . ( $link['rel'] ? ' rel="' . esc_attr( $link['rel'] ) . '"' : '' ) . ' class="heading-link">' . $link['title'] . '</a>';
	} else {
